...
import { ArrowRight, ArrowUpRight, Check, ChevronDown, ChevronLeft, ChevronRight, Facebook, FileText, Globe2, HeartHandshake, Instagram, Linkedin, Mail, MapPin, MessageCircle, Phone, Share2, ShieldCheck, Star, X, Youtube } from 'lucide-vue-next'
import { useAppStore } from '../stores/app'
import { useRequestForm } from '../composables/useRequestForm'
import { loadFirstPublicBusiness, loadPublicBusiness, loadPublicBusinessSelection, migrateLocalPublicReviews, savePublicReview } from '../services/workspace.service'
import type { Business, RequestType, Review } from '../types'
...
}); const siteLanguage = computed<'es' | 'en'>(() => app.settings.siteLanguage === 'en' ? 'en' : (baseBusiness.value.englishEnabled ? (baseBusiness.value.language ?? 'es') : 'es')); const business = computed(() => {
...
const cookieAccepted = ref(typeof window !== 'undefined' && window.localStorage.getItem('web-universal-cookies-accepted') === 'true')
...
const cookieVisible = computed(() => app.settings.cookieBanner && !cookieAccepted.value && !isUnavailable.value)
...
function acceptCookies() {
  ...
  window.localStorage.setItem('web-universal-cookies-accepted', 'true')
}
...
async function loadSite() {
  ...
  void migrateLocalPublicReviews(publicBusiness.value?.id ?? business.value.id, (publicBusiness.value as (Business & { ownerId?: string }) | null)?.ownerId)
...
const galleryPageCount = computed(() => Math.max(1, Math.ceil(business.value.gallery.length / galleryPageSize)))
const currentGallery = computed(() => business.value.gallery.slice(galleryPage.value * galleryPageSize, (galleryPage.value + 1) * galleryPageSize))
watch(() => business.value.id, () => { galleryPage.value = 0 })
function changeGalleryPage(direction: number) { galleryPage.value = Math.min(Math.max(galleryPage.value + direction, 0), galleryPageCount.value - 1) }
const reviewPage = ref(0)
const approvedReviews = computed(() => business.value.reviews.filter((review) => review.approved))
const reviewPageCount = computed(() => Math.max(1, Math.ceil(approvedReviews.value.length / galleryPageSize)))
const currentReviews = computed(() => approvedReviews.value.slice(reviewPage.value * galleryPageSize, (reviewPage.value + 1) * galleryPageSize))
watch(() => business.value.id, () => { reviewPage.value = 0 })
function changeReviewPage(direction: number) { reviewPage.value = Math.min(Math.max(reviewPage.value + direction, 0), reviewPageCount.value - 1) }

const reviewOpen = ref(false)
const reviewError = ref('')
const reviewDraft = ref({ name: '', rating: 5, text: '' })
function openReview() { reviewError.value = ''; reviewOpen.value = true }
function closeReview() { reviewOpen.value = false; reviewError.value = '' }
async function submitReview() {
  const name = reviewDraft.value.name.trim(); const text = reviewDraft.value.text.trim()
  if (!name || !text) { reviewError.value = t('Escribe tu nombre y cuéntanos cómo fue tu experiencia.', 'Enter your name and tell us about your experience.'); return }
  const currentBusiness = publicBusiness.value ?? app.selectedBusiness
  if (!currentBusiness) return
  const review: Review = { id: Date.now(), name, rating: reviewDraft.value.rating, text, createdAt: new Date().toLocaleDateString('es-ES'), approved: false }
  const ownerId = (currentBusiness as Business & { ownerId?: string }).ownerId
  if (!await savePublicReview(currentBusiness.id, review, ownerId)) { reviewError.value = t('No se ha podido guardar la reseña en la base de datos. Inténtalo de nuevo.', 'The review could not be saved to the database. Please try again.'); return }
  const reviews = [...currentBusiness.reviews, review]
  if (publicBusiness.value) publicBusiness.value = { ...publicBusiness.value, reviews }
  if (app.businesses.some((item) => item.id === currentBusiness.id)) { app.setSelected(currentBusiness.id); app.updateBusiness({ reviews }) }
  reviewDraft.value = { name: '', rating: 5, text: '' }; closeReview(); successMessage.value = t('Gracias por tu reseña. La revisaremos antes de publicarla.', 'Thank you for your review. We will check it before publishing.'); window.setTimeout(() => { successMessage.value = '' }, 4500)
}
</script>
<template>
  <main class="public-site" :class="{ dark: app.settings.darkMode, 'maintenance-mode': isUnavailable }" :style="{ '--accent': business.color }">
    <div v-if="successMessage" class="success-banner" role="status">{{ successMessage }}</div><div v-if="cookieVisible" class="cookie-banner" role="dialog" aria-label="Cookies"><div><strong>{{ t('Usamos cookies', 'We use cookies') }}</strong><span>{{ t('Usamos cookies técnicas para mejorar tu experiencia.', 'We use technical cookies to improve your experience.') }}</span></div><button class="button" :style="{ background: business.color }" @click="acceptCookies">{{ t('Aceptar', 'Accept') }}</button></div>
    ...
    <section class="proof-strip"><div><strong>{{ business.proofRatingValue || (business.reviews.some((review) => review.approved) ? '4.9/5' : '100%') }}</strong><span>{{ business.proofRatingLabel || t('Valoración de clientes', 'Customer rating') }}</span></div><div><strong>{{ business.proofAreasValue || `${business.serviceAreas.length}+` }}</strong><span>{{ business.proofAreasLabel || t('Zonas de servicio', 'Service areas') }}</span></div><div><strong>{{ business.proofResponseValue || '48h' }}</strong><span>{{ business.proofResponseLabel || t('Respuesta rápida', 'Fast response') }}</span></div><div><strong>{{ business.proofPricingValue || '✓' }}</strong><span>{{ business.proofPricingLabel || t('Precios claros', 'Clear pricing') }}</span></div></section>
    <section class="public-content"><section id="servicios" class="public-section"><span class="eyebrow" :style="{ color: business.color }">{{ t('LO QUE HACEMOS', 'WHAT WE DO') }}</span><h2>{{ t('Soluciones pensadas para ti', 'Solutions designed for you') }}</h2><p>{{ business.about }}</p><div class="card-grid"><article v-for="(service, index) in business.services" :key="service" class="service-card"><small :style="{ color: business.color }">0{{ index + 1 }}</small><Check :size="19" :style="{ color: business.color }" /><h3>{{ service }}</h3><p>{{ t('Atención profesional, clara y adaptada a lo que necesitas.', 'Professional, clear service tailored to your needs.') }}</p></article></div></section>
      <section id="nosotros" class="story public-section"><div><span class="eyebrow" :style="{ color: business.color }">{{ business.storyEyebrow || t('SOBRE', 'ABOUT') }} {{ business.name.toUpperCase() }}</span><h2>{{ business.storyTitle || t('Una forma de trabajar con sentido', 'A better way to work with purpose') }}</h2><p>{{ business.story }}</p><button class="text-button" @click="scrollToSection('servicios')">{{ t('Ver servicios', 'View services') }} <ArrowRight :size="15" /></button></div><aside><HeartHandshake :size="27" :style="{ color: business.color }" /><strong>{{ business.storyAsideTitle || t('Trato cercano, resultados reales.', 'Personal approach, real results.') }}</strong><span>{{ business.storyAsideText || t('Te acompañamos desde la primera conversación hasta el último detalle.', 'We support you from the first conversation to the final detail.') }}</span></aside></section>
      <section v-if="business.modules.gallery" id="trabajos" class="public-section"><div class="section-heading-row"><div><span class="eyebrow" :style="{ color: business.color }">{{ t('TRABAJOS E IMÁGENES', 'WORK & IMAGES') }}</span><h2>{{ t('Una muestra de nuestro trabajo', 'A sample of our work') }}</h2></div><div v-if="galleryPageCount > 1" class="gallery-controls"><button class="gallery-control" :disabled="galleryPage === 0" :aria-label="t('Imágenes anteriores', 'Previous images')" @click="changeGalleryPage(-1)"><ChevronLeft :size="17" /></button><span>{{ galleryPage + 1 }} / {{ galleryPageCount }}</span><button class="gallery-control" :disabled="galleryPage === galleryPageCount - 1" :aria-label="t('Más imágenes', 'More images')" @click="changeGalleryPage(1)"><ChevronRight :size="17" /></button></div></div><div class="gallery"><figure v-for="(image, index) in currentGallery" :key="`${image}-${galleryPage}-${index}`"><img :src="image" :alt="`${business.name} - ${t('trabajo', 'project')} ${galleryPage * galleryPageSize + index + 1}`" /><figcaption><span>{{ String(galleryPage * galleryPageSize + index + 1).padStart(2, '0') }}</span> {{ t('Resultado real', 'Real result') }}</figcaption></figure></div><p v-if="!business.gallery.length" class="public-empty">{{ t('Próximamente añadiremos trabajos a esta galería.', 'Projects will be added to this gallery soon.') }}</p></section>
      ...
      <section v-if="business.modules.reviews" id="resenas" class="public-section"><div class="section-heading-row"><div><span class="eyebrow" :style="{ color: business.color }">{{ t('RESEÑAS DE CLIENTES', 'CUSTOMER REVIEWS') }}</span><h2>{{ t('Lo que dicen nuestros clientes', 'What our clients say') }}</h2></div><div class="review-heading-actions"><div v-if="reviewPageCount > 1" class="gallery-controls"><button class="gallery-control" :disabled="reviewPage === 0" :aria-label="t('Reseñas anteriores', 'Previous reviews')" @click="changeReviewPage(-1)"><ChevronLeft :size="17" /></button><span>{{ reviewPage + 1 }} / {{ reviewPageCount }}</span><button class="gallery-control" :disabled="reviewPage === reviewPageCount - 1" :aria-label="t('Más reseñas', 'More reviews')" @click="changeReviewPage(1)"><ChevronRight :size="17" /></button></div><button class="button review-write-button" :style="{ background: business.color }" @click="openReview"><Star :size="15" /> {{ t('Dejar una reseña', 'Leave a review') }}</button></div></div><div class="card-grid"><article v-for="review in currentReviews" :key="review.id" class="review-card"><span class="stars">{{ '★'.repeat(review.rating) }}{{ '☆'.repeat(5 - review.rating) }}</span><p>“{{ review.text }}”</p><strong>{{ review.name }}</strong></article><p v-if="!approvedReviews.length" class="public-empty">{{ t('Sé la primera persona en compartir su experiencia.', 'Be the first person to share your experience.') }}</p></div></section>
    ...
    <div v-if="requestOpen" class="modal-backdrop" @click.self="closeRequest"><form class="modal" @submit.prevent="submitRequest"><button type="button" class="close" @click="closeRequest"><X :size="18" /></button><span class="eyebrow" :style="{ color: business.color }">{{ t('CONTACTO', 'CONTACT') }}</span><h2>{{ requestType === 'reserva' ? t('Solicita tu cita', 'Book your appointment') : requestType === 'presupuesto' ? t('Pide presupuesto', 'Request a quote') : t('Escríbenos', 'Write to us') }}</h2><p>{{ t('Cuéntanos lo necesario y adaptaremos la respuesta a tu tipo de servicio.', 'Tell us what you need and we will tailor the response to your type of service.') }}</p><div class="form-grid"><label>{{ t('Nombre', 'Name') }}<input v-model="draft.name" required /></label><label>{{ t('Teléfono', 'Phone') }}<input v-model="draft.phone" required /></label><label>Email<input v-model="draft.email" type="email" required /></label><label>{{ t('¿Cómo prefieres que te contactemos?', 'How would you like us to contact you?') }}<select v-model="draft.preferredContact"><option>Email</option><option>{{ t('Teléfono', 'Phone') }}</option><option>WhatsApp</option></select></label><label>{{ t('Asunto', 'Subject') }}<input v-model="draft.subject" required /></label><label>{{ t('Servicio o necesidad', 'Service or need') }}<input v-model="draft.service" :placeholder="t('Ej. corte, fuga, reforma...', 'E.g. haircut, leak, renovation...')" /></label><label>{{ t('Zona o localidad', 'Area or location') }}<input v-model="draft.area" :placeholder="t('¿Dónde sería el servicio?', 'Where would the service be?')" /></label><label v-if="requestType !== 'reserva'">{{ t('Presupuesto orientativo', 'Estimated budget') }}<select v-model="draft.budget"><option value="">{{ t('Prefiero comentarlo', 'I prefer to discuss it') }}</option><option>{{ t('Menos de 500 €', 'Under 500 €') }}</option><option>{{ t('500 € - 1.500 €', '500 € - 1,500 €') }}</option><option>{{ t('1.500 € - 5.000 €', '1,500 € - 5,000 €') }}</option><option>{{ t('Más de 5.000 €', 'Over 5,000 €') }}</option></select></label><label v-if="requestType === 'reserva'">{{ t('Fecha', 'Date') }}<input v-model="draft.date" type="date" required /></label><label v-if="requestType === 'reserva'">{{ t('Hora', 'Time') }}<input v-model="draft.time" type="time" required /></label><label class="wide">{{ t('Cuéntanos qué necesitas', 'Tell us what you need') }}<textarea v-model="draft.detail" rows="4" required :placeholder="t('Añade medidas, preferencias, fechas o cualquier detalle útil.', 'Add dimensions, preferences, dates or any useful detail.')" /></label></div><p v-if="requestError" class="request-error" role="alert">{{ requestError }}</p><button class="button" :style="{ background: business.color }" type="submit">Enviar solicitud <ArrowUpRight :size="15" /></button></form></div>
      <div v-if="reviewOpen" class="modal-backdrop" @click.self="closeReview"><form class="modal review-modal" @submit.prevent="submitReview"><button type="button" class="close" @click="closeReview"><X :size="18" /></button><span class="eyebrow" :style="{ color: business.color }">{{ t('TU EXPERIENCIA', 'YOUR EXPERIENCE') }}</span><h2>{{ t('Deja tu reseña', 'Leave your review') }}</h2><p>{{ t('Tu opinión nos ayuda a seguir mejorando. La revisaremos antes de publicarla.', 'Your feedback helps us improve. We will review it before publishing.') }}</p><label>{{ t('Tu nombre', 'Your name') }}<input v-model="reviewDraft.name" required /></label><fieldset class="rating-field"><legend>{{ t('Valoración', 'Rating') }}</legend><button v-for="value in 5" :key="value" type="button" :class="{ active: value <= reviewDraft.rating }" :aria-label="`${value} estrellas`" @click="reviewDraft.rating = value">★</button></fieldset><label>{{ t('¿Cómo fue tu experiencia?', 'How was your experience?') }}<textarea v-model="reviewDraft.text" rows="5" required /></label><p v-if="reviewError" class="request-error">{{ reviewError }}</p><button class="button" :style="{ background: business.color }" type="submit"><Star :size="15" /> {{ t('Enviar reseña', 'Send review') }}</button></form></div>
